 ...
 * 
 * This script applies the generated migration SQL directly to UAT database
 * 
 * Usage:
 *   npx tsx scripts/apply-migration-to-uat.ts
 */
...
async function applyMigration() {
  ...
  try {
    console.log('🚀 Applying Migration to UAT Database\n');
    ...
    if (!fs.existsSync(migrationPath)) {
      throw new Error(`Migration file not found: ${migrationPath}\nPlease run: npx tsx scripts/migrate-schema-qa-to-uat.ts first`);
    }
    ...
    connection = await mysql.createConnection(UAT_DB_CONFIG);
    console.log('✅ Connected to UAT database\n');

    // Split SQL into statements
    // Handle both semicolon-delimited and multi-line statements
    ...
    console.log('\n' + '='.repeat(60));
    console.log('✅ Migration applied successfully to UAT!');
    console.log('='.repeat(60));
